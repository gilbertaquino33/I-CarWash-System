...
- **`/`** — marketing landing page: hero, services, "why choose us", live
  customer testimonials, and a collapsible FAQ.
- **`/services`** — overview of the 3 real app services (Walk-In Wash, Online
  ...
  Why Choose Us, process steps, and a service-specific FAQ).
...
|---|---|
| Homepage: headline, services, why-us, steps, FAQ | `app/page.tsx` (the arrays at the top: `services`, `whyUs`, `steps`, `homeFaq`) |
...
- Wording is deliberately plain and everyday ("We watch every bay") instead
  of jargon, so ordinary customers can understand it. Keep this tone when
  adding new copy.
...
- The review form writes through Supabase with Postgres Row-Level-Security
  policies and column `check` constraints — inserts are parameterized (no SQL
  injection surface). Reviews go live at once (the policy only accepts
  `status = 'approved'` from visitors, and no one but the database owner can
